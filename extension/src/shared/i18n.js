(function initCodexOverleafI18n(root, factory) {
  ...
  const MESSAGES = {
    en: {
      ...
      diagnosticsOtTitle: 'Check OT Mirror',
      ...
      diagnosticsOtSummaryEnabled: 'OT Mirror is enabled. Codex will use the OT-warmed mirror only when matching files are fresh; otherwise it falls back to the normal project read path.',
      diagnosticsOtSummaryDisabled: 'OT Mirror is disabled, so Codex is using the normal project read path.',
      diagnosticsOtNextStep: 'Observer status describes warming availability. File scope and freshness are checked again before each run.',
      experimentalOtMenuTitle: 'OT Mirror',
      otSettingsTitle: 'Project cache',
      contextLoadingTitle: 'Context loading',
      runFeedbackFailedTitle: 'Needs attention',
      runFeedbackPartialTitle: 'Partially completed',
      runFeedbackUnconfirmedTitle: 'Write status needs review',
      runFeedbackGeneric: 'Open the run details for the reported cause and the next step.',
      runFeedbackUnconfirmed: 'Some changes may already be in Overleaf. Check the document before retrying.',
      runFeedbackUnconfirmedResult: 'Not fully confirmed. Review Overleaf before retrying.',
      runFeedbackReviewFile: 'Review {file} in Overleaf',
      runFeedbackReviewDetails: 'Review run details',
      runFeedbackDetailsUnavailable: 'Detailed steps are unavailable for this older run.',
      experimentalOtMenuSubtitle: 'Warms recent edits to the active text file locally. File scope and freshness are checked before reuse.',
      experimentalOtConfirmTitle: 'Enable OT Mirror?',
      experimentalOtConfirmMessage: 'Observes the active text editor and warms its local mirror. This is read-only with respect to Overleaf and does not upload assets. Each run checks file scope and freshness before reuse; normal project reading remains the fallback.',
      experimentalOtConfirmEnable: 'Turn on',
      experimentalOtEnabledToast: 'OT Mirror requested for this project. Its status shows whether warming is available.',
      ...
      forkRunTitle: 'Fork a new conversation from this turn',
      forkRunUnavailable: 'This turn cannot be forked because its Codex thread position is unavailable.',
      forkRunFailed: 'Could not fork this conversation.',
      forkRunDone: 'Forked conversation created.',
      forkSessionTitle: '{title} (fork)',
      copyConclusionTitle: 'Copy conclusion',
      copyConclusionDone: 'Conclusion copied',
      copyConclusionFailed: 'Could not copy conclusion',
      ...
      tasks: 'Tasks',
      placeholder: 'Ask Codex anything. Type @ to add files as context',
      mode: 'Mode',
      ...
      attachFiles: 'Attach files (PDF, images)',
      fileProtectionTitle: 'File protection',
      sessionEvictTitle: 'Session limit reached',
      sessionEvictMessage: 'You have 20 sessions. Creating a new one will permanently delete the oldest session "{title}" and all of its history.',
      sessionEvictConfirm: 'Delete oldest & create',
      sessionSwitchRunningTitle: 'A run is still in progress',
      sessionSwitchRunningMessage: 'The current session is still running. If you leave now, the run continues in the background and its result is saved to that session.',
      sessionSwitchRunningConfirm: 'Leave & continue in background',
      diffHunkRevert: 'Undo decision',
      runsTruncatedNote: 'Showing the most recent 20 runs — older runs were trimmed.',
      eventsTruncatedNote: 'Earlier steps of this run were trimmed.',
      newSessionTooltip: 'Start a fresh conversation — the current session\u2019s context will not carry over',
      storageTitle: 'History & storage',
      softwareUpdatesTitle: 'Software updates',
      softwareUpdatesHelp: 'Automatically checks signed stable releases for the Extension and Native Host.',
      checkForUpdates: 'Check for updates',
      providerSettingsTitle: 'Model providers',
      providerSettingsExperimental: 'Third-party experimental',
      providerSettingsHelp: 'Configure experimental third-party model APIs. Compatibility varies by provider and gateway.',
      providerSettingsConfigure: 'Configure',
      providerSettingsBuiltin: 'Built-in Codex is active',
      providerSettingsLoading: 'Checking current project provider…',
      storageUsageLoading: 'Calculating usage\u2026',
      storageClearAll: 'Clear all history\u2026',
      storageClearAllHelp: 'Removes every stored session, run and audit record for all projects. Project settings and rules are kept.',
      runTurnNavLabel: 'Jump to a turn\u2026',
      historyTitle: 'Change history',
      historyHelp: 'What Codex wrote to this project, per run. Filter by file name or task text.',
      runSearchPlaceholder: 'Search this session\u2019s runs\u2026',
      undoFileSelectTitle: 'Undo which files?',
      undoFileSelectConfirm: 'Undo selected',
      undoFileSelectAll: 'Select all',
      privacyTitle: 'Privacy',
      governanceBroadReadonlyWarning: '"{pattern}" matches every file — Codex will not be able to write anything. Saved; narrow it if unintended.',
      governanceWritableAllowlistNote: 'Writable allowlist active — files outside these patterns cannot be written.',
      atMenuLoading: 'Loading project files…',
      atMenuFileSubtitle: 'Add as focus context',
      atMenuCompileLogSubtitle: 'Attach the latest compile log',
      diffHunkLocation: 'Line {line} · {count} lines',
      diagnosticsDotOk: 'Diagnostics — all checks passed',
      diagnosticsDotWarn: 'Diagnostics — something needs attention',
      diagnosticsDotFail: 'Diagnostics — a check failed, click to inspect',
      diagnosticsDotUnknown: 'Diagnostics',
      ...
      experimentalOtWarmMirror: 'OT warm mirror',
      experimentalOtWarmMirrorShort: 'OT',
      experimentalOtWarmMirrorTitle: 'Read-only local text warming. File scope and freshness are checked before reuse; normal project reading remains the fallback.',
      otStatus: 'OT status',
      otFreshFiles: 'Files with recent OT evidence',
      otFallback: 'Read fallback',
      otStatusOff: 'Off',
      otStatusStarting: 'Connecting',
      otStatusObserving: 'Observing current editor',
      otStatusUnavailable: 'Unavailable',
      otStatusStale: 'Warming paused; awaiting verification',
      otStatusInconsistent: 'Needs verification',
      otFailureBridge: 'The page connection needs to be restored. Reload the extension and Overleaf before retrying.',
      otFailureObserver: 'The editor observer is unavailable. The next run will try normal project reading.',
      otFailureEditor: 'The active editor could not be read. Warming is paused; the next run will try normal project reading.',
      otFailureFileLimit: 'This file exceeds the 1 MiB warming limit. The next run will try normal project reading.',
      otFailureQueueLimit: 'The mirror queue reached its limit. Warming is paused; the next run will try normal project reading.',
      otFailureExpired: 'The observation is too old to reuse. The next run will try normal project reading.',
      otFailureMirror: 'The mirror update was not fully confirmed. The next run will try normal project reading.',
      otFailureGeneric: 'Mirror warming could not continue. The next run will try normal project reading.',
      otFailureNextStep: 'Resolve the reported cause, then enable OT again. If the page connection is unavailable, reload the extension and Overleaf first.',
      ...
      queueNextInput: 'Queue follow-up',
      queuedInputQueued: 'Queued',
      queuedInputPaused: 'Queue paused',
      queuedInputGuiding: 'Sending guidance',
      queuedInputExecuting: 'Running',
      queuedInputGuide: 'Guide',
      queuedInputGuideTitle: 'Send this message into the active Codex turn now.',
      queuedInputGuideUnavailable: 'The active turn is not ready for guidance yet.',
      queuedInputGuideFailed: 'Guidance could not be delivered. The message remains queued.',
      queuedInputGuided: 'Follow-up guidance was delivered to the active Codex turn.',
      queuedInputAdded: 'Follow-up queued. It will run after this task settles.',
      queuedInputResume: 'Resume',
      queuedInputRemove: 'Remove',
      queuedInputAttachmentsUnsupported: 'Queued follow-ups currently support text and @context. Remove binary attachments before queuing.',
      queuedInputError_empty_input: 'Enter a follow-up first.',
      queuedInputError_input_too_long: 'This follow-up is too long to queue.',
      queuedInputError_queue_full: 'The follow-up queue is full.',
      queuedInputError_queue_too_large: 'The follow-up queue reached its storage limit.',
      contextTitle: '@ Context',
      refreshFileList: 'Refresh file list',
      contextStatus: 'Type @ to add files, or click ＋: files, attachments (PDF/images), @compile-log.',
      contextLoadingFiles: 'Reading available @files...',
      contextSyncTreeLoading: 'Reading the project file tree.',
      contextSyncExactLoading: 'Available files are ready. Completing the full ZIP file list.',
      contextSyncExactReady: 'The complete project file list is ready.',
      contextSyncPartial: 'Available files are shown. The complete ZIP file list is unavailable; refresh to retry.',
      ...
      contextDefaultWholeProject: 'Default is the whole project. Choose files or add @compile-log. For a section, name it in your request.',
      preloadProjectContextTitle: 'Preload project context',
      preloadProjectContextDescription: 'Fetch the exact project file list in the background after an Overleaf project becomes idle.',
      ...
      emptyRunsHint: 'Type @ to add files, / for commands.',
      ...
      processedCancelled: 'Cancelled {elapsed}',
      ...
      recentProjects_welcome: 'Continue working',
      recentProjects_focusEyebrow: 'Last conversation',
      recentProjects_historyHeading: 'Other recent conversations',
      recentProjects_historyHint: 'Select to continue',
      recentProjects_projectConversations: "View this project's conversations",
      recentProjects_allProjects: 'All projects',
      recentProjects_showAllConversations: 'View all {count} conversations',
      recentProjects_unstarted: 'Not started \u00b7 {count}',
      recentProjects_unstartedHint: 'Empty conversations are kept here, without deleting their records.',
      recentProjects_noStarted: 'No conversation has been started yet. Empty conversations are available below.',
      recentProjects_savedDraft: 'Saved draft, ready to continue.',
      recentProjects_attention: 'Needs attention',
      recentProjects_unavailableRecords: 'Records with unavailable project links',
      recentProjects_loadingHistory: 'Loading conversations...',
      recentProjects_loadFailed: 'Conversation history could not be read. Please retry.',
      recentProjects_continueHint: 'Open this conversation without automatically starting a task.',
      recentProjects_more: 'More options',
      recentProjects_continue: 'Continue',
      recentProjects_openProject: 'Open project',
      recentProjects_loadingConversation: 'Loading conversation...',
      recentProjects_noConversation: 'No saved conversation',
      recentProjects_noRuns: 'No runs yet',
      recentProjects_previewUnavailable: 'Conversation unavailable',
      recentProjects_previewUnavailableHint: 'Saved conversation could not be loaded.',
      recentProjects_conversationGone: 'This conversation is no longer available. Choose another conversation from the refreshed list.',
      recentProjects_continueFailed: 'Could not open the conversation. Please try again.',
      recentProjects_welcome_subtitle: 'Conversations saved in this browser',
      recentProjects_empty: 'No conversations saved in this browser yet. Open an Overleaf project and start a Codex conversation.',
      recentProjects_sessions_empty: 'No saved conversations in this project',
      recentProjects_sessions_toggle: "Show or manage this project's conversations",
      recentProjects_sessions_action: 'All conversations',
      recentProjects_sessions_action_open: 'Hide conversations',
      recentProjects_clearProject: 'Delete this project\'s conversations',
      recentProjects_clearProject_title: 'Delete all Codex sessions for {project}?',
      recentProjects_clearProject_message: 'This permanently deletes all {count} plugin sessions, run history, and matching plugin-local Codex thread history for {project}. It will not modify Overleaf files or global Codex history.',
      recentProjects_clearProject_confirm: 'Delete all',
      recentProjects_clearProject_running: 'This project may still have a running Codex task. Cancel it before deleting all sessions.',
      recentProjects_clearProject_done: 'Deleted {count} session(s) from {project}.',
      recentProjects_clearProject_partial: 'Deleted {removed} of {count} sessions. The remaining records are still visible; retry to remove them.',
      recentProjects_clearProject_historyPartial: 'Sessions were deleted, but {count} plugin-local Codex thread history item(s) could not be cleared.',
      ...
      recentProjects_settingsTitle: 'General Settings',
      recentProjects_settingsSubtitle: 'Manage Codex preferences and local history across all projects.',
      recentProjects_badge_pending: 'Last run pending',
      recentProjects_badge_accepted: 'Last changes accepted',
      recentProjects_badge_rejected: 'Last changes undone',
      recentProjects_badge_needs_review: 'Last run needs review',
      recentProjects_badge_running: 'Last recorded as running',
      recentProjects_badge_completed: 'Last run completed',
      recentProjects_badge_failed: 'Last run failed',
      recentProjects_badge_interrupted: 'Last run may have been interrupted',
      recentProjects_badge_stale: 'Last run status unconfirmed',
      recentProjects_unnamed: 'Project \u00b7 {prefix}',
      recentProjects_zombieRunningNote: 'Marked as running, but it may be a leftover from an interrupted run.',
      recentProjects_zombieDeleteConfirm: 'This session is marked as running. If no other tab is actually running it, you can delete it anyway.',
      recentProjects_retryScope: 'Retry',
      recentProjects_degraded_hint: 'Could not identify the Overleaf account on this page, so the project list is hidden to avoid mixing accounts. This usually resolves on a reload.',
      recentProjects_sessionsCount: '{count} session(s)',
      recentProjects_welcome_stats: '{projects} projects \u00b7 {sessions} Codex sessions',
      recentProjects_badge_background_completed: 'Last run completed in background',
      recentProjects_badge_needs_review_after_navigation: 'Last run needs review',
      recentProjects_badge_abandoned_after_navigation: 'Last run stopped after navigation'
    },
    zh: {
      ...
      diagnosticsOtTitle: '检查 OT Mirror',
      ...
      diagnosticsOtSummaryEnabled: 'OT Mirror 已开启。只有匹配文件是新鲜状态时，Codex 才会使用 OT 预热镜像；否则会回退到常规项目读取路径。',
      diagnosticsOtSummaryDisabled: 'OT Mirror 已关闭，Codex 会使用常规项目读取路径。',
      diagnosticsOtNextStep: '观测状态说明预热是否可用；每次任务开始前仍会重新校验文件范围和新鲜度。',
      experimentalOtMenuTitle: 'OT Mirror',
      otSettingsTitle: '项目缓存',
      contextLoadingTitle: '上下文加载',
      runFeedbackFailedTitle: '需要处理',
      runFeedbackPartialTitle: '部分完成',
      runFeedbackUnconfirmedTitle: '写入状态待确认',
      runFeedbackGeneric: '可在本轮详情中查看具体原因和后续处理方式。',
      runFeedbackUnconfirmed: '部分改动可能已经写入 Overleaf，重试前请先检查文档。',
      runFeedbackUnconfirmedResult: '结果尚未完全确认，请检查 Overleaf 后再决定是否重试。',
      runFeedbackReviewFile: '在 Overleaf 检查 {file}',
      runFeedbackReviewDetails: '查看运行步骤',
      runFeedbackDetailsUnavailable: '这条历史记录没有可展开的详细步骤。',
      experimentalOtMenuSubtitle: '在本地预热当前文本文件的最近改动，复用前重新校验文件范围和新鲜度。',
      experimentalOtConfirmTitle: '开启 OT Mirror？',
      experimentalOtConfirmMessage: '观测当前文本编辑器并预热本地镜像。此功能不会写回 Overleaf，也不负责上传资产。每次任务复用前都会校验文件范围和新鲜度，不满足条件时尝试常规项目读取。',
      experimentalOtConfirmEnable: '开启',
      experimentalOtEnabledToast: '已为当前项目请求开启 OT Mirror，可用情况见状态提示。',
      ...
      forkRunTitle: '从此轮分叉一个新对话',
      forkRunUnavailable: '该轮缺少可定位的 Codex thread 节点，无法分叉。',
      forkRunFailed: '无法分叉此对话。',
      forkRunDone: '已创建分叉对话。',
      forkSessionTitle: '{title}（分叉）',
      copyConclusionTitle: '复制结论',
      copyConclusionDone: '已复制结论',
      copyConclusionFailed: '无法复制结论',
      ...
      newSession: '新建会话',
      tasks: '任务',
      placeholder: '问 Codex 任何事。输入 @ 添加文件上下文',
      mode: '模式',
      writeMode: '写入模式',
      modeAsk: '只问不改',
      ...
      addContext: '添加 @ 上下文',
      attachFiles: '添加附件（PDF、图片）',
      fileProtectionTitle: '文件保护',
      sessionEvictTitle: '会话数已达上限',
      sessionEvictMessage: '当前已有 20 个会话。新建将永久删除最早的会话「{title}」及其全部历史。',
      sessionEvictConfirm: '删除最早并新建',
      sessionSwitchRunningTitle: '仍有运行进行中',
      sessionSwitchRunningMessage: '当前会话还在运行。现在离开的话，该运行会在后台继续，结果仍会记录到原会话。',
      sessionSwitchRunningConfirm: '离开并后台继续',
      diffHunkRevert: '撤销该决定',
      runsTruncatedNote: '仅显示最近 20 轮——更早的轮次已精简。',
      eventsTruncatedNote: '本轮更早的步骤已精简。',
      newSessionTooltip: '开启全新对话——当前会话的上下文不会带入',
      storageTitle: '历史与存储',
      softwareUpdatesTitle: '软件更新',
      softwareUpdatesHelp: '自动检查 Extension 与 Native Host 的签名稳定版本。',
      checkForUpdates: '检查更新',
      providerSettingsTitle: '模型服务',
      providerSettingsExperimental: '第三方实验性',
      providerSettingsHelp: '配置实验性第三方模型 API；兼容性取决于具体模型服务与网关。',
      providerSettingsConfigure: '配置',
      providerSettingsBuiltin: '当前使用内置 Codex',
      providerSettingsLoading: '正在读取当前项目的模型服务…',
      storageUsageLoading: '正在计算占用…',
      storageClearAll: '清空全部历史…',
      storageClearAllHelp: '删除所有项目已存储的全部会话、运行与审计记录。项目设置与规则会保留。',
      runTurnNavLabel: '跳到某一轮…',
      historyTitle: '变更历史',
      historyHelp: 'Codex 每一轮写入本项目的记录。可按文件名或任务内容过滤。',
      runSearchPlaceholder: '搜索本会话的轮次…',
      undoFileSelectTitle: '撤销哪些文件？',
      undoFileSelectConfirm: '撤销所选',
      undoFileSelectAll: '全选',
      privacyTitle: '隐私',
      governanceBroadReadonlyWarning: '「{pattern}」匹配所有文件——Codex 将无法写入任何内容。已保存；若非有意请收窄范围。',
      governanceWritableAllowlistNote: '已设置可写白名单——白名单之外的文件都不可写。',
      atMenuLoading: '正在加载项目文件…',
      atMenuFileSubtitle: '加入重点上下文',
      atMenuCompileLogSubtitle: '附上最新编译日志',
      diffHunkLocation: '第 {line} 行起 · 共 {count} 行',
      diagnosticsDotOk: '诊断——全部检查通过',
      diagnosticsDotWarn: '诊断——有需要留意的项',
      diagnosticsDotFail: '诊断——有检查未通过，点击查看',
      diagnosticsDotUnknown: '诊断',
      reasoningLabel: '推理强度',
      speedLabel: '速度',
      requireReviewing: '留痕',
      ...
      experimentalOtWarmMirror: 'OT 预热镜像',
      experimentalOtWarmMirrorShort: 'OT',
      experimentalOtWarmMirrorTitle: '只读本地文本预热；复用前校验文件范围和新鲜度，常规项目读取作为回退路径。',
      otStatus: 'OT 状态',
      otFreshFiles: '近期有 OT 校验证据的文件',
      otFallback: '读取回退',
      otStatusOff: '未开启',
      otStatusStarting: '正在连接',
      otStatusObserving: '正在观测当前编辑器',
      otStatusUnavailable: '暂不可用',
      otStatusStale: '预热暂缓，等待重新校验',
      otStatusInconsistent: '需重新校验',
      otFailureBridge: '页面连接需要恢复，请先重新加载扩展并刷新 Overleaf，再重试。',
      otFailureObserver: '编辑器观察器暂不可用，下次任务将尝试常规项目读取。',
      otFailureEditor: '当前编辑器内容尚不可读，预热已暂停；下次任务将尝试常规项目读取。',
      otFailureFileLimit: '文件超过 1 MiB 预热上限，下次任务将尝试常规项目读取。',
      otFailureQueueLimit: '镜像队列达到容量上限，预热已暂停；下次任务将尝试常规项目读取。',
      otFailureExpired: '观测记录已过期，下次任务将尝试常规项目读取。',
      otFailureMirror: '镜像更新尚未完整确认，下次任务将尝试常规项目读取。',
      otFailureGeneric: '镜像预热未能继续，下次任务将尝试常规项目读取。',
      otFailureNextStep: '处理所显示的原因后重新开启 OT；若页面连接不可用，请先重新加载扩展并刷新 Overleaf。',
      ...
      send: '发送',
      cancelRun: '中断当前任务',
      queueNextInput: '加入后续队列',
      queuedInputQueued: '已排队',
      queuedInputPaused: '队列已暂停',
      queuedInputGuiding: '正在发送引导',
      queuedInputExecuting: '正在运行',
      queuedInputGuide: '立即引导',
      queuedInputGuideTitle: '立即把这条消息发送到当前 Codex turn。',
      queuedInputGuideUnavailable: '当前 turn 尚未进入可引导状态。',
      queuedInputGuideFailed: '引导消息未送达，消息已保留在队列中。',
      queuedInputGuided: '后续引导已发送到当前 Codex turn。',
      queuedInputAdded: '后续任务已排队，将在当前任务完整结束后执行。',
      queuedInputResume: '恢复队列',
      queuedInputRemove: '删除',
      queuedInputAttachmentsUnsupported: '当前排队消息支持文本和 @context；请先移除二进制附件。',
      queuedInputError_empty_input: '请先输入后续任务。',
      queuedInputError_input_too_long: '这条后续任务太长，无法排队。',
      queuedInputError_queue_full: '后续任务队列已满。',
      queuedInputError_queue_too_large: '后续任务队列已达到存储上限。',
      contextTitle: '@ 上下文',
      refreshFileList: '刷新文件列表',
      contextStatus: '输入 @ 添加文件，或点 ＋：文件、附件（PDF/图片）、@compile-log。',
      contextLoadingFiles: '正在读取可添加的 @文件...',
      contextSyncTreeLoading: '正在读取项目文件树。',
      contextSyncExactLoading: '可用文件已经显示，正在补齐完整 ZIP 文件列表。',
      contextSyncExactReady: '完整项目文件列表已经就绪。',
      contextSyncPartial: '当前已显示可用文件，完整 ZIP 文件列表暂不可用；可刷新重试。',
      ...
      contextDefaultWholeProject: '默认使用整个项目。可选择文件或添加 @compile-log；处理章节时请在请求中注明章节名称。',
      preloadProjectContextTitle: '预加载项目上下文',
      preloadProjectContextDescription: 'Overleaf 项目进入空闲状态后，在后台读取精确的项目文件列表。',
      ...
      processedCancelled: '已取消 {elapsed}',
      ...
      recentProjects_welcome: '继续工作',
      recentProjects_focusEyebrow: '上次对话',
      recentProjects_historyHeading: '其他最近对话',
      recentProjects_historyHint: '点击条目继续',
      recentProjects_projectConversations: '查看该项目的全部对话',
      recentProjects_allProjects: '所有项目',
      recentProjects_showAllConversations: '查看全部 {count} 段对话',
      recentProjects_unstarted: '未开始 · {count}',
      recentProjects_unstartedHint: '空白对话收纳在这里，记录不会被删除。',
      recentProjects_noStarted: '尚无已开始的对话，空白对话可在下方展开。',
      recentProjects_savedDraft: '草稿已保存，可继续补充。',
      recentProjects_attention: '需要查看',
      recentProjects_unavailableRecords: '项目链接不可用的记录',
      recentProjects_loadingHistory: '正在加载对话…',
      recentProjects_loadFailed: '暂时无法读取对话历史，请重试。',
      recentProjects_continueHint: '仅打开这段对话，不会自动启动任务。',
      recentProjects_more: '更多选项',
      recentProjects_continue: '继续',
      recentProjects_openProject: '打开项目',
      recentProjects_loadingConversation: '正在加载对话…',
      recentProjects_noConversation: '暂无保存的对话',
      recentProjects_noRuns: '尚未运行任务',
      recentProjects_previewUnavailable: '暂时无法读取对话',
      recentProjects_previewUnavailableHint: '已保存的对话暂时无法读取。',
      recentProjects_conversationGone: '该对话已不可用，请从更新后的列表中选择其他对话。',
      recentProjects_continueFailed: '暂时无法打开对话，请重试。',
      recentProjects_welcome_subtitle: '当前浏览器保存的对话',
      recentProjects_empty: '当前浏览器还没有保存的对话。打开一个 Overleaf 项目，与 Codex 开始对话即可。',
      recentProjects_sessions_empty: '该项目还没有保存的对话',
      recentProjects_sessions_toggle: '查看或管理该项目的对话',
      recentProjects_sessions_action: '全部对话',
      recentProjects_sessions_action_open: '收起对话',
      recentProjects_clearProject: '删除该项目的全部对话',
      recentProjects_clearProject_title: '删除 {project} 的全部 Codex 会话？',
      recentProjects_clearProject_message: '这会永久删除 {project} 的全部 {count} 个插件会话、运行历史及对应的插件本地 Codex 线程历史。不会修改 Overleaf 文件或全局 Codex 历史。',
      recentProjects_clearProject_confirm: '全部删除',
      recentProjects_clearProject_running: '该项目可能仍有 Codex 任务在运行。请先取消任务，再删除全部会话。',
      recentProjects_clearProject_done: '已删除 {project} 的 {count} 个会话。',
      recentProjects_clearProject_partial: '已删除 {count} 个会话中的 {removed} 个。剩余记录仍会显示，可重试删除。',
      recentProjects_clearProject_historyPartial: '会话已删除，但有 {count} 条插件本地 Codex 线程历史未能清除。',
      ...
      recentProjects_settingsTitle: '通用设置',
      recentProjects_settingsSubtitle: '管理适用于所有项目的 Codex 偏好设置与本地历史记录。',
      recentProjects_badge_pending: '上次运行待处理',
      recentProjects_badge_accepted: '上次修改已接受',
      recentProjects_badge_rejected: '上次修改已撤回',
      recentProjects_badge_needs_review: '上次运行需要检查',
      recentProjects_badge_running: '上次记录为运行中',
      recentProjects_badge_completed: '上次运行已完成',
      recentProjects_badge_failed: '上次运行失败',
      recentProjects_badge_interrupted: '上次运行可能已中断',
      recentProjects_badge_stale: '上次运行状态未确认',
      recentProjects_unnamed: '项目 · {prefix}',
      recentProjects_zombieRunningNote: '标记为运行中，但可能是被中断后的残留。',
      recentProjects_zombieDeleteConfirm: '该会话标记为运行中。如果确认没有其他标签页在运行它，可以直接删除。',
      recentProjects_retryScope: '重试',
      recentProjects_degraded_hint: '无法识别本页的 Overleaf 账号，为避免账号间串数据已隐藏项目列表。通常刷新即可恢复。',
      recentProjects_sessionsCount: '{count} 个会话',
      recentProjects_welcome_stats: '{projects} 个项目 · {sessions} 个 Codex 会话',
      recentProjects_badge_background_completed: '上次运行已在后台完成',
      recentProjects_badge_needs_review_after_navigation: '上次运行需要检查',
      recentProjects_badge_abandoned_after_navigation: '上次运行已在离开页面后停止'
    }
  };
